 ...
 */
class RebuttalForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'merchant_dashboard_rebuttal_form';
  }
...
public function submitForm(array &$form, FormStateInterface $form_state) {
  ...
  try {
    $node->save();
    $this->messenger()->addMessage($this->t('Your rebuttal has been submitted successfully.'));
    $form_state->setRedirect('<current>');
  }
  catch (\Exception $e) {
    $this->messenger()->addError($this->t('An error occurred while saving the rebuttal. Please try again.'));
    \Drupal::logger('merchant_dashboard')->error('Error saving rebuttal node: @error', ['@error' => $e->getMessage()]);
  }
}
}